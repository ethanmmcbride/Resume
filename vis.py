...
# Quick Sort Time Complexity: O(n log(n)) when the pivot 
#   divides the array into roughly equal parts. 
# Worst case O(n^2) if not.
...
# Radix Sort Time Complexity: O(n*d) 
# where n is the size of the array 
# and d is the size of the largest number
def radix_sort(arr):
    max1 = max(arr)
    exp = 1
    while max1 // exp > 0:
